/**
 * CLI script to clear all DynamoDB tables (except Book) in dev environment
 * Creates automatic backup before deletion
 * 
 * Usage:
 *   npx tsx scripts/clear-db-dev.ts
 * 
 * Requirements:
 ...
 * 
 * WARNING: This script will DELETE all data from tables (except Book)!
...
async function main(): Promise<void> {
  console.log('🚀 Starting DynamoDB cleanup for dev environment...\n');
  ...
  // Filter out excluded tables and only include dev tables
  const env = process.env.AWS_BRANCH || 'dev';
  ...
  const backupDir = join(__dirname, `../backups/dev-${timestamp}`);
