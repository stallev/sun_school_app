...
### Визуальные макеты
- **[WIREFRAMES.md](../../../ui_ux/WIREFRAMES.md)** - визуальные макеты страниц
  - Раздел 2.1 Страница входа - макет страницы аутентификации

### Дизайн-система
- **[DESIGN_SYSTEM.md](../../../ui_ux/DESIGN_SYSTEM.md)** - дизайн-система проекта, компоненты форм
...
**Действия:**
- [x] Установить aws-amplify: `npm install aws-amplify`
- [x] Проверить совместимость версии с Next.js 15.5.9
- [x] Проверить совместимость с AWS Amplify Gen 1 (не Gen 2!)
...
**Действия:**
- [x] Создать файл `lib/amplify/config.ts`
- [x] Импортировать конфигурацию из `amplifyconfiguration.json`
- [x] Настроить Amplify.configure() с конфигурацией
- [x] Создать утилиту для получения конфигурации
- [x] Убедиться, что конфигурация загружается корректно
...
**Действия:**
- [x] Изучить wireframe страницы входа из [WIREFRAMES.md](../../../ui_ux/WIREFRAMES.md)
- [x] Создать `app/(auth)/auth/page.tsx`
- [x] Создать Client Component формы входа с:
  - Полями Email и Password
  - Валидацией через Zod
  - Обработкой ошибок
  - Кнопкой входа
  - Loading state
- [x] Использовать компоненты Shadcn UI (Form, Input, Button)
- [x] Интегрировать с AWS Amplify Auth (signIn)
...
**Действия:**
- [x] Создать `app/actions/auth.ts`
- [x] Реализовать Server Action `signIn`:
  - Принимает email и password
  - Валидирует через Zod
  - Вызывает Amplify Auth signIn
  - Возвращает результат или ошибку
- [x] Реализовать Server Action `signOut`:
  - Вызывает Amplify Auth signOut
  - Очищает сессию
- [x] Реализовать Server Action `getCurrentUser`:
...
**Действия:**
- [x] Создать `middleware.ts` в корне проекта
- [x] Реализовать проверку JWT токена из cookies
- [x] Реализовать редирект неавторизованных пользователей на `/auth`
- [x] Реализовать проверку роли для админских страниц
- [x] Реализовать редирект Teacher с админских страниц на `/grades/my`
- [x] Использовать Next.js middleware API
...
**Действия:**
- [x] Создать `lib/utils/auth.ts`
- [x] Реализовать функцию `getSession()`:
  - Получает текущую сессию из Cognito
  - Возвращает информацию о пользователе
- [x] Реализовать функцию `getUserRole()`:
  - Получает роль пользователя из Cognito групп
  - Возвращает роль (TEACHER, ADMIN, SUPERADMIN)
- [x] Реализовать функцию `isAuthenticated()`:
...
**Действия:**
- [x] Обновить Header компонент для отображения информации о пользователе
- [x] Добавить кнопку выхода в Header
- [x] Реализовать обработку выхода
- [x] Обновить Sidebar для скрытия пунктов меню для неавторизованных пользователей
- [x] Убедиться, что layout работает с аутентификацией
...
**Действия:**
- [x] Реализовать обработку ошибок входа:
  - Неверный email/password
  - Пользователь не подтвержден
  - Пользователь заблокирован
  - Другие ошибки Cognito
- [x] Отображать понятные сообщения об ошибках пользователю
- [x] Использовать Toast уведомления для ошибок
...
**Действия:**
- [x] Протестировать вход с правильными учетными данными
- [x] Протестировать вход с неверными учетными данными
- [x] Протестировать защиту маршрутов (попытка доступа без авторизации)
- [x] Протестировать проверку ролей (Teacher пытается зайти на админскую страницу)
- [x] Протестировать выход
- [x] Протестировать сохранение сессии (refresh страницы)
...
**Действия:**
- [x] Убедиться, что проверка сессии не блокирует рендеринг
- [x] Оптимизировать загрузку конфигурации Amplify
- [x] Проверить, что нет ненужных запросов к Cognito
- [x] Оптимизировать размер bundle для auth компонентов
