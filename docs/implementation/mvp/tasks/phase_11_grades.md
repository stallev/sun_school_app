...
<task>
Создай Server Actions для управления группами в файле `actions/grades.ts`. Реализуй все CRUD операции с проверкой прав доступа и валидацией через Zod схемы.
</task>
...
**Действия:**
- [ ] Создать `actions/grades.ts` с директивой `'use server'`
...
- [ ] Реализовать `getGrade(id)` - получение группы с проверкой доступа
...
- [ ] Использовать валидацию через Zod схемы для всех входных данных
...
- [ ] Использовать Data Access Layer для получения данных через Server Actions
- [ ] Добавить проверку прав доступа на сервере
...
- [ ] Добавить кнопки действий: "Создать урок", "Настройки" (Admin only), "Расписание"
...
- [ ] Использовать утилиту в Server Actions (createGrade, updateGrade, deleteGrade, getGrade, listGrades)
...
- [ ] Протестировать удаление группы (Admin)
- [ ] Протестировать просмотр группы (Teacher и Admin)
- [ ] Протестировать список групп (Teacher видит только свои, Admin - все)
- [ ] Протестировать проверку прав доступа для всех операций
- [ ] Протестировать валидацию форм (все поля, все ошибки)
