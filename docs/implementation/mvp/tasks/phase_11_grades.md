...
**Действия:**
- [x] Создать `actions/grades.ts` с директивой `'use server'`
...
- [ ] Реализовать функцию `checkGradeAccess(userId, gradeId, userRole)`:
  - ⚠️ **Частично выполнено**: Функция `checkTeacherGradeAccess(userId, gradeId)` реализована в `actions/grades.ts`, но:
...
- [ ] Реализовать функцию `getUserGrades(userId)` для получения групп Teacher
  - ⚠️ **Частично выполнено**: Функция `getTeacherGradeIds(userId)` реализована в `actions/grades.ts`, но:
    - Возвращает только ID групп, а не полные объекты
    - Находится в `actions/grades.ts`, а не в утилите
- [x] Использовать утилиту в Server Actions (createGrade, updateGrade, deleteGrade, getGrade, listGrades)
  ...
  - ⚠️ **Проблема**: Функции определены внутри `actions/grades.ts`, а не импортируются из утилиты
...
- ⚠️ **Дублирование кода**: Функции определены внутри `actions/grades.ts` вместо централизованной утилиты
