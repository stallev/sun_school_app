...
- [ ] Создать страницу `/grades/:gradeId/issue-bricks` для выдачи кирпичиков группе:
  - Список учеников с информацией:
    - ФИО ученика (кликабельно для раскрытия истории выдачи)
    - Набрано кирпичиков: X (равно количеству баллов за учебный год)
    - Выдано кирпичиков: Y (сумма всех выданных кирпичиков из истории)
    - Остаток: Z (набрано - выдано)
  - Accordion компонент для истории выдачи (по аналогии с FAQ блоком):
    ...
    - Использовать компонент Accordion из Shadcn UI (`components/ui/accordion.tsx`)
  - Форма выдачи кирпичиков для каждого ученика:
    ...
    - Валидация: нельзя выдать больше, чем остаток
  - Состояния:
    - Если остаток = 0: форма выдачи скрыта, показывается "✅ Все кирпичики выданы"
    - Если история пуста: в accordion показывается "История выдачи пуста"
...
- [ ] Создать функцию `getPupilBricksHistory(pupilId, academicYearId)` для получения истории выдачи кирпичиков ученика:
  - Получение истории с фильтрацией по учебному году
  - Возврат истории с датами выдачи в хронологическом порядке (от новых к старым)
  - История должна быть отсортирована по полю `issuedAt` в порядке убывания
  - История используется для отображения в accordion компоненте на странице выдачи кирпичиков
- [ ] Добавить обработку ошибок

**Документация:**
- <CRITICAL>[BRICKS_ISSUE_IMPLEMENTATION_PLAN.md](./BRICKS_ISSUE_IMPLEMENTATION_PLAN.md) - план выполнения GraphQL схемы</CRITICAL>
- <CRITICAL>[app_functionality.md](../../../app_functionality.md) - раздел 4.14 Выдача кирпичиков</CRITICAL>
- [app_functionality.md](../../../app_functionality.md) - раздел 4.9 Личные данные ученика
...
**Важно:** 
- После реализации GraphQL схемы (см. BRICKS_ISSUE_IMPLEMENTATION_PLAN.md):
  - Используйте query `bricksIssuesByPupilIdAndIssuedAt` для получения истории выдачи
  - История должна быть отсортирована по `issuedAt` в порядке убывания (новые сверху)
  - История отображается в accordion компоненте на странице выдачи кирпичиков

**Критерии приемки:**
- История выдачи кирпичиков сохраняется корректно
- Можно получить историю выдачи кирпичиков ученика
- История отсортирована по дате (от новых к старым)
- История отображается в accordion компоненте на странице выдачи кирпичиков
