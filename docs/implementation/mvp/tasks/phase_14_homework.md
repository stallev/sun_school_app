...
- **[app_functionality.md](../../../app_functionality.md)** - единственный источник истины для функциональных требований
  - Раздел 4.8 Проверка домашних заданий - описание функционала проверки ДЗ

### Пользовательские сценарии
- **[USER_FLOW.md](../../../user_flows/USER_FLOW.md)** - общие пользовательские сценарии и flow-диаграммы
  - Раздел 4.3 Массовая проверка домашних заданий - flow проверки ДЗ
- **[TEACHER_FLOWS.md](../../../user_flows/TEACHER_FLOWS.md)** - детальные flow для преподавателей
  - Раздел 4.1 Последовательная проверка ДЗ - детальный flow проверки
  - Раздел 4.2 Просмотр сводной таблицы - flow просмотра результатов

### Визуальные макеты
- **[WIREFRAMES.md](../../../ui_ux/WIREFRAMES.md)** - визуальные макеты страниц и компонентов
  - Раздел 3.2 Управление уроками - макеты для проверки ДЗ
  - Homework Check - макет интерфейса проверки

### API и валидация
- **[SERVER_ACTIONS.md](../../../api/SERVER_ACTIONS.md)** - спецификация Server Actions API
  - Раздел Homework - API для управления проверками ДЗ
- **[VALIDATION.md](../../../api/VALIDATION.md)** - схемы валидации Zod
  - Раздел Homework Schemas - схемы валидации для проверок ДЗ
...
### Task 14.02: Предотвращение дубликатов проверок домашних заданий

<context>
<CRITICAL>Это критически важная задача для корректности системы!</CRITICAL> В системе обнаружены дубликаты проверок домашних заданий - для одной пары (lessonId, pupilId) создается несколько записей. Это приводит к неправильному подсчету прогресса проверки (например, 417% вместо максимум 100%). Необходимо реализовать механизм предотвращения дубликатов и очистить существующие дубликаты.

⚠️ **Важно:** Подробное описание проблемы, анализ причин и варианты решения описаны в issue документе: [HOMEWORK_CHECK_DUPLICATES.md](../../../issues/HOMEWORK_CHECK_DUPLICATES.md)
</context>

<task>
Реализуй механизм предотвращения дубликатов проверок домашних заданий. Создай функцию upsert для проверок, обнови все места создания проверок для использования upsert, и создай скрипт для очистки существующих дубликатов.

⚠️ **Важно:** Перед началом работы изучи issue документ [HOMEWORK_CHECK_DUPLICATES.md](../../../issues/HOMEWORK_CHECK_DUPLICATES.md) для понимания проблемы и рекомендуемого решения.
</task>

<constraints>
- Должна быть реализована функция upsert (update or insert) для проверок
- Функция должна проверять существование проверки по паре (lessonId, pupilId) перед созданием
- Если проверка существует - обновлять, если нет - создавать
- Все места создания проверок должны использовать upsert вместо create
- Должен быть создан скрипт для очистки существующих дубликатов
- Скрипт должен оставлять запись с самым поздним updatedAt для каждой пары (lessonId, pupilId)
</constraints>

<thinking>
Прежде чем приступить к реализации:
1. Изучи [HOMEWORK_CHECK_DUPLICATES.md](../../../issues/HOMEWORK_CHECK_DUPLICATES.md) для понимания проблемы и решения
2. Изучи текущую реализацию создания проверок в `src/lib/db/mutations.ts`
3. Изучи GraphQL схему для понимания структуры индексов
4. Определи структуру функции upsert и функции проверки существования
5. Продумай логику очистки дубликатов
6. Только после этого реализуй решение
</thinking>

**Действия:**
- [ ] Изучить issue документ [HOMEWORK_CHECK_DUPLICATES.md](../../../issues/HOMEWORK_CHECK_DUPLICATES.md)
- [ ] Создать функцию `getHomeworkCheckByLessonAndPupil(lessonId, pupilId)` в `src/lib/db/queries.ts`:
  - Использовать GSI-1: lessonId-pupilId-index для запроса
  - Query с lessonId = PK и pupilId = SK
  - Вернуть первую найденную запись или null
- [ ] Создать функцию `upsertHomeworkCheck(input)` в `src/lib/db/mutations.ts`:
  - Проверить существование через `getHomeworkCheckByLessonAndPupil`
  - Если существует - обновить через `updateHomeworkCheck`
  - Если не существует - создать через `createHomeworkCheck`
- [ ] Обновить все места создания проверок для использования `upsertHomeworkCheck`:
  - Server Actions в `src/actions/homework.ts` (если существует)
  - Компоненты проверки ДЗ (если существуют)
  - Массовое создание проверок
- [ ] Создать скрипт `scripts/cleanup-homework-check-duplicates.ts`:
  - Получить все проверки из DynamoDB
  - Группировать по (lessonId, pupilId)
  - Для каждой пары с дубликатами найти запись с самым поздним updatedAt
  - Удалить остальные записи
  - Вывести отчет о проделанной работе
- [ ] Протестировать upsert функцию:
  - Создание новой проверки
  - Обновление существующей проверки
  - Проверка отсутствия дубликатов

**Документация:**
- <CRITICAL>[HOMEWORK_CHECK_DUPLICATES.md](../../../issues/HOMEWORK_CHECK_DUPLICATES.md) - детальное описание проблемы, анализ причин и рекомендуемое решение</CRITICAL>
- [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md) - схема HomeworkCheck и индексы
- [DYNAMODB_SCHEMA.md](../../../database/DYNAMODB_SCHEMA.md) - структура таблицы HomeworkChecks и GSI-1
- [SERVER_ACTIONS.md](../../../api/SERVER_ACTIONS.md) - раздел Homework для понимания API
- Context7: AWS DynamoDB GSI queries документация
- **Код реализации:**
  - [src/lib/db/queries.ts](../../../../src/lib/db/queries.ts) - функции запросов
  - [src/lib/db/mutations.ts](../../../../src/lib/db/mutations.ts) - функции мутаций
  - [src/lib/db/amplify.ts](../../../../src/lib/db/amplify.ts) - Data Access Layer

**Критерии приемки:**
- Функция `getHomeworkCheckByLessonAndPupil` создана и работает корректно
- Функция `upsertHomeworkCheck` создана и работает корректно
- Все места создания проверок используют `upsertHomeworkCheck`
- Скрипт очистки дубликатов создан и работает
- Новые дубликаты не создаются
- Процент проверки не превышает 100%

<output_format>
После выполнения задачи должен быть реализован механизм предотвращения дубликатов проверок. Функция upsert должна работать корректно, все места создания проверок должны использовать upsert, существующие дубликаты должны быть очищены. Новые дубликаты не должны создаваться.
</output_format>

---

### Task 14.03: Создание таблицы массовой проверки
...
### Task 14.04: Реализация автоматического расчета баллов
...
### Task 14.05: Реализация логики получения домика
...
### Task 14.06: Создание страницы проверки ДЗ
...
### Task 14.07: Реализация сохранения результатов
...
### Task 14.08: Реализация редактирования проверки ДЗ
...
### Task 14.09: Тестирование функционала проверки ДЗ
