...
<CONSTRAINT>Массовая проверка ДЗ обязательна. Расчет баллов должен учитывать настройки группы. Логика получения домика должна работать корректно.</CONSTRAINT>
</critical_instructions>
</requirements>
...
<output_format>
После выполнения задачи должна быть реализована логика получения домика. Функция должна работать корректно, определяя получение домика на основе всех включенных параметров. Индикатор домика должен отображаться в таблице.
</output_format>
...
- Определение домика должно происходить автоматически
...
- [ ] Интегрировать определение домика (Task 14.04)
...
- Определение домика работает
- Сохранение работает корректно
- Обработка ошибок работает

<output_format>
После выполнения задачи должна быть создана страница проверки ДЗ. Страница должна интегрировать все компоненты: таблицу массовой проверки, автоматический расчет баллов и определение домика. Сохранение результатов должно работать корректно.
</output_format>
...
- Статус домика должен обновляться автоматически при изменении параметров
...
- Статус домика обновляется автоматически
- Расчет баллов обновляется автоматически
- Обновление работает через Server Action
- Обработка ошибок работает

<output_format>
После выполнения задачи должна быть создана страница редактирования проверки ДЗ. Данные должны предзаполняться корректно, статус домика и расчет баллов должны обновляться автоматически при изменении параметров. Обновление должно работать через Server Action.
</output_format>
...
<task>
Проведи комплексное тестирование всего функционала проверки домашних заданий. Протестируй массовую проверку, расчет баллов, получение домика, сохранение результатов и редактирование проверки для всех сценариев.
</task>
...
<output_format>
После выполнения задачи должно быть проведено комплексное тестирование всего функционала проверки ДЗ. Все функции должны работать корректно, расчет баллов должен работать для всех настроек группы, получение домика должно работать для всех сценариев. Система должна быть готова к использованию.
</output_format>
