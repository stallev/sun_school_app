...
## Описание фазы
Реализация управления учебными годами: создание, завершение, активация завершенного года, автоматическое определение активного года, блокировка создания уроков без активного года.
...
- **[app_functionality.md](../../../app_functionality.md)** - единственный источник истины для функциональных требований
  - Раздел 4.1 Страница группы - описание функционала управления учебными годами
...
- **[ADMIN_FLOWS.md](../../../user_flows/ADMIN_FLOWS.md)** - детальные flow для администраторов
  - Раздел 6.1 Завершение учебного года - сценарий завершения года

### Визуальные макеты
- **[WIREFRAMES.md](../../../ui_ux/WIREFRAMES.md)** - визуальные макеты страниц и компонентов
  - Раздел 3.1 Управление группами - макеты для учебных годов

### API и валидация
- **[SERVER_ACTIONS.md](../../../api/SERVER_ACTIONS.md)** - спецификация Server Actions API
  - Раздел Academic Years - API для управления учебными годами
- **[VALIDATION.md](../../../api/VALIDATION.md)** - схемы валидации Zod
  - Раздел Academic Year Schemas - схемы валидации для учебных годов
...
<task>
Создай Server Actions для управления учебными годами в файле `src/actions/academicYears.ts`. Реализуй все операции с проверкой прав доступа и логикой определения активного года.
</task>
...
**Действия:**
- [x] Создать `src/actions/academicYears.ts` с директивой `'use server'`
- [x] Реализовать `createAcademicYear(input)` - создание года (Admin only)
- [x] Реализовать `updateAcademicYear(input)` - обновление года (Admin only)
- [x] Реализовать `getActiveAcademicYear(gradeId)` - получение активного года группы
- [x] Реализовать `listAcademicYears(gradeId)` - список годов группы
- [x] Реализовать `completeAcademicYear(gradeId)` - завершение года (Admin only)
- [x] Реализовать `activateAcademicYear(academicYearId)` - активация завершенного года (Admin only, Task 12.05)
...
**Действия:**
- [x] Создать утилиту `lib/utils/academicYears.ts`
- [x] Реализовать функцию `getActiveYear(gradeId)`:
  - Ищет год со статусом ACTIVE для группы через amplifyData
  - Возвращает активный год или null
  - Обрабатывает ошибки
- [x] Использовать функцию в Server Actions (getActiveAcademicYear, createLesson)
- [x] Использовать функцию в компонентах при необходимости
...
- Показывай статус каждого года (ACTIVE, FINISHED) с визуальными индикаторами
...
**Действия:**
- [x] Создать страницу `app/(private)/grades/[gradeId]/academic-years/page.tsx`
- [x] Реализовать Server Component для загрузки данных через Server Action
- [x] Отобразить список учебных годов группы
- [x] Показать статус каждого года (ACTIVE, FINISHED) с Badge компонентами
- [x] Добавить кнопку создания нового года (Admin only)
- [x] Добавить кнопку завершения активного года (Admin only)
- [x] Добавить кнопку активации завершенного года (Admin only, для годов со статусом FINISHED, Task 12.05)
- [x] Использовать компоненты Shadcn UI (Table, Card, Badge, Button)
...
<task>
Реализуй Server Action для глобального завершения всех активных учебных годов. Создай функцию, которая завершает все активные годы, изменяя их статус на FINISHED. Добавь UI для выполнения действия.
</task>
...
**Действия:**
- [x] Реализовать Server Action `completeAllAcademicYears()` (Admin only)
- [x] Добавить проверку прав доступа через Cognito
- [x] Реализовать логику завершения всех активных учебных годов
- [x] Изменить статус всех активных годов на FINISHED через amplifyData
- [x] Добавить обработку ошибок
- [x] Добавить UI для выполнения действия (кнопка с Dialog подтверждением)
...
<output_format>
После выполнения задачи должна быть реализована функция глобального завершения всех активных учебных годов. Функция должна работать корректно, завершая все активные годы и изменяя их статус на FINISHED. UI должен показывать подтверждение перед выполнением действия.
</output_format>

---

### Task 12.05: Реализация активации завершенного учебного года

<context>
Активация завершенного учебного года позволяет Admin восстановить завершенный год в активный статус. Это полезная функция для исправления ошибок или возврата к предыдущему учебному году. Правильная реализация с проверкой бизнес-правил критически важна для корректности данных.
</context>

<task>
Реализуй Server Action для активации завершенного учебного года. Создай функцию, которая активирует завершенный год, изменяя его статус на ACTIVE. Добавь проверку, что в группе нет другого активного года. Добавь UI для выполнения действия.
</task>

<constraints>
- Функция должна быть доступна только для Admin
- Функция должна проверять, что год имеет статус FINISHED
- Функция должна проверять, что в группе нет другого ACTIVE года
- Если в группе есть другой ACTIVE год, должна возвращаться ошибка с понятным сообщением
- Изменение статуса должно быть атомарным
- Должна быть добавлена проверка прав доступа
- UI должен показывать подтверждение перед выполнением действия
- Обработка ошибок должна быть реализована
</constraints>
...
Прежде чем приступить к реализации:
1. Изучи MVP_SCOPE.md раздел 2.2.2 для понимания бизнес-логики активного года
2. Определи структуру функции и логику активации завершенного года
3. Продумай проверку наличия другого ACTIVE года в группе
4. Определи UI для выполнения действия (кнопка с подтверждением)
5. Только после этого реализуй функцию
</thinking>

**Действия:**
- [x] Реализовать Server Action `activateAcademicYear(academicYearId)` (Admin only)
- [x] Добавить проверку прав доступа через Cognito
- [x] Добавить проверку, что год имеет статус FINISHED
- [x] Добавить проверку, что в группе нет другого ACTIVE года через утилиту `getActiveYear(gradeId)`
- [x] Если в группе есть другой ACTIVE год, возвращать ошибку с понятным сообщением
- [x] Изменить статус года на ACTIVE через amplifyData
- [x] Добавить обработку ошибок
- [x] Добавить UI для выполнения действия (кнопка с Dialog подтверждением на странице управления учебными годами)
- [x] Кнопка активации должна отображаться только для годов со статусом FINISHED
...
- Context7: AWS Amplify Gen 1 amplifyData документация
- **Код реализации:**
  - [src/lib/utils/academicYears.ts](../../../../src/lib/utils/academicYears.ts) - утилита getActiveYear для проверки активного года
  - [src/lib/db/amplify.ts](../../../../src/lib/db/amplify.ts) - Data Access Layer (amplifyData)
  - [src/lib/auth/cognito.ts](../../../../src/lib/auth/cognito.ts) - проверка прав доступа

**Критерии приемки:**
- Активация завершенного года работает корректно
- Проверка наличия другого ACTIVE года работает
- Проверка прав доступа работает
- UI с подтверждением работает
- Обработка ошибок работает
...
<output_format>
После выполнения задачи должна быть реализована функция активации завершенного учебного года. Функция должна работать корректно, проверяя наличие другого ACTIVE года в группе и изменяя статус завершенного года на ACTIVE. UI должен показывать подтверждение перед выполнением действия.
</output_format>

---

### Task 12.06: Блокировка создания уроков без активного года
...
**Действия:**
- [x] Обновить Server Action `createLesson`:
...
### Task 12.07: Тестирование функционала учебных годов

<context>
Тестирование функционала учебных годов является финальным этапом фазы. Комплексное тестирование всех функций, логики активного года, активации завершенного года и блокировки создания уроков критически важно для обеспечения качества системы.
</context>

<task>
Проведи комплексное тестирование всего функционала управления учебными годами. Протестируй создание, завершение, активацию завершенного года, определение активного года и блокировку создания уроков для всех сценариев.
</task>
...
- Активация завершенного года должна быть протестирована
...
**Действия:**
- [x] Протестировать создание учебного года (Admin)
- [x] Протестировать завершение года (Admin)
- [ ] Протестировать глобальное завершение всех активных годов (Admin)
- [x] Протестировать активацию завершенного года (Admin):
  - Активация года со статусом FINISHED
  - Попытка активации года, когда в группе уже есть ACTIVE год (должна возвращаться ошибка)
  - Активация года с проверкой, что другой ACTIVE год завершается автоматически (если требуется)
