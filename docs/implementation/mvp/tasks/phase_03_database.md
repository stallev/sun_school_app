...
- [x] Создать базовые типы согласно [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md):
  - **Основные типы:**
...
- [x] Добавить все необходимые enum типы:
...
<task>
Добавь все связи между типами в GraphQL Schema. Реализуй связи многие-ко-многим через промежуточные типы. ⚠️ **Важно:** Директивы `@belongsTo` и `@hasMany` удалены из схемы для устранения циклических зависимостей CloudFormation. Связи доступны через queries с использованием индексов.
</task>

<constraints>
- Связи многие-ко-многим должны быть реализованы через промежуточные типы
- Все связи должны соответствовать ERD
- ⚠️ **Критически важно:** Директивы `@belongsTo` и `@hasMany` удалены из схемы
- Связи доступны только через queries с использованием индексов (GSI)
- Все индексы для связей должны быть созданы
- Схема должна соответствовать ERD
</constraints>

<thinking>
Прежде чем приступить к реализации:
1. Изучи GRAPHQL_SCHEMA.md раздел 1.4 и раздел Relationships детально
2. Изучи SCHEMA_DIFFERENCES.md для понимания причин удаления связей
3. Изучи ERD.md - все связи между сущностями
4. Определи, какие связи многие-ко-многим требуют промежуточных типов
5. Определи, какие индексы необходимы для получения связанных данных через queries
6. Только после этого добавляй связи в schema (без @belongsTo/@hasMany, только индексы)
</thinking>

**Действия:**
- [x] Добавить типы для связей многие-ко-многим:
  - `UserGrade` для связи User-Grade (с индексами byUserId и byGradeId)
  - `LessonGoldenVerse` для связи Lesson-GoldenVerse (с индексами byLessonId и byGoldenVerseId)
  - `PupilAchievement` для связи Pupil-Achievement (с индексами byPupilId и byAchievementId)
  - `FamilyMember` для связи Family-Pupil (с индексами byFamilyId и byPupilId)
  - `UserFamily` для связи User-Family (с индексами byUserId и byFamilyId)
- [x] ⚠️ **Важно:** Директивы `@belongsTo` и `@hasMany` удалены из схемы
- [x] Добавить индексы для получения связанных данных через queries:
  - Индексы для всех связей (byUserId, byGradeId, byLessonId, byPupilId и т.д.)
  - Все связи доступны через queries с использованием этих индексов

**Документация:**
- <CRITICAL>[GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md) - раздел 1.4 и раздел Relationships</CRITICAL>
...
**Критерии приемки:**
- Все связи определены в schema (без @belongsTo/@hasMany)
- Связи многие-ко-многим реализованы через промежуточные типы
- Все индексы для связей созданы
- Связи доступны через queries с использованием индексов
- Схема соответствует ERD

<output_format>
После выполнения задачи все связи должны быть определены в schema без директив @belongsTo и @hasMany. Связи многие-ко-многим должны быть реализованы через промежуточные типы с индексами, и все связи должны быть доступны через queries с использованием индексов.
</output_format>
...
**Действия:**
- [x] Добавить @auth директивы для каждого типа согласно [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md)
- [x] Настроить правила авторизации:
...
- [x] Проверить, что все типы имеют правильные @auth директивы
...
**Действия:**
- [x] Добавить @model директиву к каждому типу, который должен быть таблицей
- [x] Настроить Partition Keys и Sort Keys через @index директивы согласно [DYNAMODB_SCHEMA.md](../../../database/DYNAMODB_SCHEMA.md)
- [x] Пример: `@model` с `@index(name: "byGrade", sortKeyFields: ["lessonDate"])` на полях
- [x] Убедиться, что ключи соответствуют access patterns
...
**Действия:**
- [x] **⭐ КРИТИЧЕСКИ ВАЖНО:** Добавить поле `gradeId` в тип HomeworkCheck для поддержки GSI-3
...
- [x] Добавить GSI через @index директивы согласно [DYNAMODB_SCHEMA.md](../../../database/DYNAMODB_SCHEMA.md)
- [x] Создать GSI1 для каждого типа (если требуется)
- [x] Создать GSI2 для каждого типа (если требуется)
- [x] **⭐ КРИТИЧЕСКИ ВАЖНО:** Создать GSI-3 для HomeworkChecks (gradeId-createdAt-index) для поддержки аналитики post-MVP
...
- [x] Убедиться, что GSI поддерживают необходимые access patterns
...
**Действия:**
- [x] Проверить автоматическую генерацию queries через @model (Amplify автоматически генерирует queries для всех @model типов)
  - `getUser(id: ID!): User` - автоматически генерируется
  - `listGrades: [Grade]` - автоматически генерируется
  - `getGrade(id: ID!): Grade` - автоматически генерируется
  - `listLessons(gradeId: ID!): [Lesson]` - автоматически генерируется через GSI
  - И другие queries согласно схеме - автоматически генерируются
- [x] Настроить @auth для queries (через @auth на типах)
- [x] Добавить фильтрацию и пагинацию где необходимо (автоматически поддерживается через Amplify)
...
**Действия:**
- [x] Проверить автоматическую генерацию mutations через @model (Amplify автоматически генерирует mutations для всех @model типов)
  - `createUser(input: CreateUserInput!): User` - автоматически генерируется
  - `updateUser(input: UpdateUserInput!): User` - автоматически генерируется
  - `createGrade(input: CreateGradeInput!): Grade` - автоматически генерируется
  - `updateGrade(input: UpdateGradeInput!): Grade` - автоматически генерируется
  - И другие mutations согласно схеме - автоматически генерируются
- [x] Настроить @auth для mutations (через @auth на типах)
- [x] Добавить Input типы для всех mutations (автоматически генерируются через Amplify)
...
**Действия:**
- [x] Проверить синтаксис schema: `amplify api gql-compile`
- [x] Убедиться, что нет синтаксических ошибок
- [x] Проверить, что все типы правильно связаны
- [x] Убедиться, что все @auth директивы корректны
...
- [x] Подтвердить создание ресурсов: `Yes`
- [x] Дождаться завершения создания таблиц DynamoDB
- [x] Проверить статус: `amplify status`
- [x] Убедиться, что все таблицы созданы в AWS Console
...
**Действия:**
- [x] Открыть AWS Console -> DynamoDB
- [x] Проверить наличие всех таблиц
- [x] Проверить структуру каждой таблицы (ключи, индексы)
- [x] Убедиться, что GSI созданы корректно
- [x] Проверить настройки таблиц (read/write capacity, billing mode)
...
**Действия:**
- [x] Создать файл с данными всех 66 книг Библии (JSON или TypeScript массив)
- [x] Создать Server Action `src/actions/books.ts` с функцией `seedBooks()` для заполнения таблицы
- [x] Создать CLI скрипт `scripts/seed-books-cli.ts` для выполнения через AWS CLI
- [x] Выполнить скрипт после создания таблиц
- [x] Проверить, что все 66 книг созданы
...
- [x] Протестировать простой query (например, `listBooks`)
- [x] Протестировать простую mutation (например, `createBook`)
- [x] Проверить работу авторизации
- [x] Создать инструкции для тестирования (см. [GRAPHQL_API_TESTING.md](./GRAPHQL_API_TESTING.md))
...
**Действия:**
- [x] Обновить [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md) если были изменения
- [x] Обновить [SCHEMA_DIFFERENCES.md](../../../database/SCHEMA_DIFFERENCES.md) если были новые изменения
- [x] Задокументировать любые отклонения от первоначальной схемы
- [x] Создать заметки о важных решениях по моделированию данных

**Документация:**
- [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md)
- [SCHEMA_DIFFERENCES.md](../../../database/SCHEMA_DIFFERENCES.md) - различия между текущей и документированной схемой
...
- [GRAPHQL_SCHEMA.md](../../../database/GRAPHQL_SCHEMA.md) - GraphQL Schema
- [SCHEMA_DIFFERENCES.md](../../../database/SCHEMA_DIFFERENCES.md) - Различия между текущей и документированной схемой, причины удаления @belongsTo/@hasMany
