...
╠══════════════════════════════════════════════════════════╣
║                                                          ║
║  📚 Младшая группа (6-8 лет)  [➕ Новый урок]  [📅 Расписание]  [⚙️ Настройки] ║
║  ════════════════════════════════════════                ║
║                                                          ║
...
**Страница добавления/редактирования события:** `/grades/:gradeId/schedule/new` или `/grades/:gradeId/schedule/:eventId/edit`
```
┌──────────────────────────────────────────┐
│ Добавить событие                  [✕]   │
├──────────────────────────────────────────┤
...
║  Описание урока (опционально, rich text):                      ║
  ...
  ║  │  Поддерживаемые форматы: изображения (jpg, png, gif),   ║
  ║  │  PDF, документы (doc, docx). Максимальный размер: 10MB  ║
  ║  │  на файл. Хранение в AWS S3 Storage.                     ║
  ║  │                                                           ║
  ║  │  Загруженные файлы:                                      ║
...
**Страница "Создать новый золотой стих":** `/golden-verses/new`
```
┌───────────────────────────────────────────────────┐
...
- Кнопка архивирования скрыта (функционал перенесён в Post-MVP)
...
Переход на страницу проверки ДЗ (см. следующий раздел)
...
7. Переход на страницу проверки ДЗ
8. Заполняет данные
9. Сохраняет
10. Видит обновлённую таблицу (⚠️ пропал, появились оценки)
11. При необходимости делится данными через скриншот или обращается к администратору для выгрузки (экспорт — Post-MVP)
...
**Последовательная проверка дз (полноэкранная страница):**
```
╔══════════════════════════════════════════════════════════════════════╗
...
║ -------------------------------------------------------------------- ║
║ 👤 Попова Виктория (10 лет)      |   Урок #6: Жертвоприношение       ║
...
║     Баллы: [0] [1] [[2]]   Подсказки: 0 — не знает • 1 — с подсказкой║
║                                • 2 — наизусть                        ║
...
║ [✓ Был(а)]   [ Не был(а) ]                                           ║
║ -------------------------------------------------------------------- ║
║ Комментарий преподавателя (опционально): [____________________]      ║
║ -------------------------------------------------------------------- ║
...
║                                                                           ║
║    ┌───┬───────────┬───────────────────┬────┬─────┬──────┬────────┬──────┬──────┐ ║
  ║  │ № │   Дата    │ Тема урока        │При.│ ЗС*│ Тест │Тетрадь │Спевка│Баллы│ ║
  ║  │   │           │                   │    │ *только если showGoldenVerses=true│
║  ├───┼───────────┼───────────────────┼────┼─────┼──────┼────────┼──────┼──────┤ ║
║  │ 1 │01.09.2024 │ Сотворение мира   │ ✅ │ 6/6 │  9   │   8    │  ✅  │  23 │ ║
║  ├───┼───────────┼───────────────────┼────┼─────┼──────┼────────┼──────┼──────┤ ║
║  │ 2 │08.09.2024 │ Адам и Ева        │ ✅ │ 5/6 │  8   │   7    │  ✅  │  22 │ ║
║  ├───┼───────────┼───────────────────┼────┼─────┼──────┼────────┼──────┼──────┤ ║
║  │ 3 │15.09.2024 │ Каин и Авель      │ ❌ │  -  │  -   │   -    │  ❌  │   0 │ ║
║  ├───┼───────────┼───────────────────┼────┼─────┼──────┼────────┼──────┼──────┤ ║
║  │ 4 │22.09.2024 │ Ной и потоп       │ ✅ │ 6/6 │  10  │   9    │  ✅  │  25 │ ║
║  ├───┼───────────┼───────────────────┼────┼─────┼──────┼────────┼──────┼──────┤ ║
║  │...│    ...    │        ...        │ ...│ ... │ ...  │  ...   │ ...  │ ... │ ║
║  └───┴───────────┴───────────────────┴────┴─────┴──────┴────────┴──────┴──────┘ ║
║                                                                           ║
...
║  ├───────────────┼───────────────────────────┼────────┼──────────┼───┤  ║
║  │ Быт. 1:1      │ В начале сотворил Бог... │   67   │04.09.24 │✏️ │  ║
║  └───────────────┴───────────────────────────┴────────┴──────────┴───┘  ║
║                                                                           ║
...
**Модальное окно "Просмотр стиха" (только для просмотра):**
```
┌───────────────────────────────────────────────────┐
│ Быт. 22:14                                [✕]    │
├───────────────────────────────────────────────────┤
...
**Wireframe:** См. [WIREFRAMES.md](../ui_ux/WIREFRAMES.md) - раздел "Выдача кирпичиков"

**Wireframe:**
```
╔═══════════════════════════════════════════════════════════════════════════╗
║ 🏠 Главная > Группы > Младшая группа > Выдача кирпичиков                 ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
...
**Страница "Добавить/Редактировать преподавателя":** `/teachers/new` или `/teachers/:id/edit`
```
┌────────────────────────────────────────────────────┐
...
│                                                    │
│  Назначение групп: *                               │
│  (Преподаватель может быть назначен в несколько групп) │
...
│  [✓ Активный]  [ Неактивный]                       │
│                                                    │
│  💡 Один преподаватель может преподавать в разных │
│     группах одновременно (например, в Младшей и   │
│     Средней группах).                              │
│                                                    │
...
**Страница "Создать/Редактировать группу":** `/grades/new` или `/grades/:gradeId/edit`
```
┌───────────────────────────────────────────────────────┐
...
║  │                 │  │                 │                 ║
║  │ Семья:          │  │ Семья:          │                 ║
║  │ Ивановы (Иванов А.В.) │  │ Петровы (Петров П.И.) │      ║
║  │                 │  │                 │                 ║
║  │ ✅ Активный      │  │ ✅ Активный      │                 ║
║  │                 │  │                 │                 ║
║  │ [Просмотр]      │  │ [Просмотр]      │                 ║
...
**Страница "Добавить/Редактировать ученика":** `/pupils/new` или `/pupils/:id/edit`
```
┌─────────────────────────────────────────────────────┐
...
**Страница "Добавить/Редактировать семью":** `/families/new` или `/families/:id/edit`
```
┌──────────────────────────────────────────────────┐
...
║  Действия:                                                            ║
║                                                                       ║
║  [Завершить текущий год 2024-2025]   [Создать новый год 2025-2026]   ║
╠═══════════════════════════════════════════════════════════════════════╣
```


║  ℹ️ После завершения года:                                            ║
║  • Все группы переходят на 🟡 FINISHED статус                         ║
║  • Уроки становятся read-only для всех групп                          ║
